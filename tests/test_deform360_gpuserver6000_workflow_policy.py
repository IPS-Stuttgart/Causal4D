...
def test_file_change_dispatcher_is_hosted_and_fixed() -> None:
    ...
    assert '"diff", "--name-only", before, after' in text
    assert "forced pushes cannot dispatch reviewed Deform360 jobs" in text
    assert "initial pushes cannot dispatch reviewed Deform360 jobs" in text
    assert "push after revision does not match the checked-out head" in text
...
def test_reset_request_is_source_only_on_gpuserver4090() -> None:
    ...
    assert request == {
        ...
        "request_id": "2026-08-31-gpuserver4090-reset-mechanics-source-v2",
    }
