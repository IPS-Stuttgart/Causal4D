#!/usr/bin/env python3
"""Bind retained Deform360 source masks to fresh source-only audit artifacts.

The processed ``mask_refined.h5`` files already exist on gpuserver6000 from the
published 001-rope public-data pipeline. This script does not rerun SAM2. It
hashes those retained files, recomputes first-frame multiview consistency, and
records empty-frame diagnostics so downstream centerline extraction can use the
historical masks with explicit byte-level provenance.
"""
...
def main() -> int:
    ...
    try:
        ...
        preflight = load_deform360_preflight(args.preflight_json)
        ...
        for camera in episode_cameras(episode_dir):
            mask_path = episode_dir / camera / "mask_refined.h5"
            video_path = episode_dir / camera / "undistorted.mp4"
            if mask_path.is_file() and video_path.is_file():
                candidate_cameras.append(camera)
        candidate_cameras.sort()
        ...
        view_payload["claim_boundary"] = (
            ...
            "that already existed in the published 001-rope processed root."
        )
        from causal4d_public.deform360_sam2_views import sam2_view_audit_sha256
        ...
        mask_payload: dict[str, Any] = {
            ...
            "claim_boundary": (
                ...
                "freshly recomputed QA. It does not assert that SAM2 was rerun in "
                "this workflow."
            ),
        }
